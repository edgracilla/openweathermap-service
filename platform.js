...
Platform.init = function () {
    ...
    process.on('message', (m) => {
        if (m.type === 'ready')
            ...
            this.emit('data', m.requestId, m.data);
 ...
 * @param {string} requestId The request id from the platform
 * @param {string} result The result of the web service or API call
 * @param callback
 */
Platform.prototype.sendResult = function (requestId, result, callback) {
    callback = callback || function () {
        };

    setImmediate(() => {
        if (!result || typeof result === 'undefined') result = '{}';
        if (!isString(requestId)) return callback(new Error('Kindly specify the request id generated by the platform for this service request.'));
